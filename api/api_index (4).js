...
function matchId(n) {
  const nm = n.replace(/댐$/, '').replace(/다목적$/, '').replace(/\s/g, '')
  for (const [id, m] of Object.entries(DAM_META)) {
    const mn = m.name.replace(/댐$/, '').replace(/\s/g, '')
...
async function fetchKwater() {
  ...
  if (!text.startsWith('{')) throw new Error('K-water 응답 오류: ' + text.slice(0, 80))
  const raw = JSON.parse(text)?.response?.body?.items?.item
  if (!raw) throw new Error('K-water 데이터 없음')
  return Array.isArray(raw) ? raw : [raw]
}

async function loadCache(supabase, hours) {
...
async function handleAll(req, res, supabase) {
  // 디버그 모드
  if (req.query?.raw === '1') {
    try {
  ...
  // 1) 1시간 캐시
  try {
    const c = await loadCache(supabase, 1)
    if (c) { rows = c; source = 'supabase_cache' }
  } catch(e) {}
  ...
  if (!rows) {
    try {
      ...
      supabase && supabase.from('dam_realtime').upsert(toSave, { onConflict: 'dam_id' }).catch(() => {})
      source = 'kwater_live'
    } catch(e) {
      // 3) K-water 실패 → 24시간 stale 캐시
      try {
        const s = await loadCache(supabase, 24)
        if (s) { rows = s; source = 'supabase_stale' }
      } catch(e2) {}
      if (!rows) return res.status(503).json({ error: 'K-water API 호출 실패', detail: e.message })
    }
  }
...
module.exports = async (req, res) => {
  ...
  // /api 와 /api/all 모두 처리
  return handleAll(req, res, supabase)
}
