...
// 댐 이름 매칭 (다목적 등 suffix 제거 후 비교)
function matchId(kwaterName) {
  const nm = kwaterName
    .replace(/댐$/, '').replace(/다목적$/, '').replace(/용수$/, '').replace(/\s/g, '').trim()
  for (const [id, meta] of Object.entries(DAM_META)) {
    const mn = meta.name
      .replace(/댐$/, '').replace(/다목적$/, '').replace(/\s/g, '').trim()
    if (mn === nm || mn.includes(nm) || nm.includes(mn)) return id
  }
  return null
}

// K-water API 두 엔드포인트 병렬 호출
async function fetchKwater() {
  const base   = `https://apis.data.go.kr/B500001/dam`
  const params = `serviceKey=${KWATER_KEY}&numOfRows=100&pageNo=1&_type=json`

  const [res1, res2] = await Promise.allSettled([
    fetch(`${base}/sluicePresentCondition/?${params}`).then(r => r.json()),
    fetch(`${base}/damStoragePresentCondition/?${params}`).then(r => r.json()),
  ])

  const toArr = v => v ? (Array.isArray(v) ? v : [v]) : []
  const arr1 = toArr(res1.status === 'fulfilled' ? res1.value?.response?.body?.items?.item : null)
  const arr2 = toArr(res2.status === 'fulfilled' ? res2.value?.response?.body?.items?.item : null)

  // arr1 우선, arr2로 보완
  const merged = [...arr1]
  for (const item of arr2) {
    const id = matchId(item.damNm || '')
    if (id && !arr1.find(i => matchId(i.damNm || '') === id)) merged.push(item)
  }

  if (!merged.length) throw new Error('K-water 응답 없음')
  return merged
}
...
module.exports = async (req, res) => {
  ...
  // ★ 디버그 모드: ?raw=1 이면 K-water 원본 댐 이름 반환
  if (req.query && req.query.raw === '1') {
    try {
      const items = await fetchKwater()
      const names = items.map(i => i.damNm).sort()
  ...
  try {
    ...
    if (cached) { rows = cached; source = 'supabase_cache' }
  } catch (e) {}
