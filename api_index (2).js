...
async function fetchKwater() {
  ...
  let json
  try { json = JSON.parse(text) } catch(e) { throw new Error('K-water 응답 파싱 실패: ' + text.slice(0,100)) }
  const raw = json?.response?.body?.items?.item
  if (!raw) throw new Error('K-water 데이터 없음: ' + JSON.stringify(json).slice(0,200))
